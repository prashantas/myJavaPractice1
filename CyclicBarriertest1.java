...
public class CyclicBarrierTest1 {
	...
	public static void main(String[] args) {
		...
		CyclicBarrier cb = new CyclicBarrier(4, new Aggragator(list));
     
		new Task(cb,1,5,list);
		new Task(cb,6,10,list);
		new Task(cb,11,15,list);
		System.out.println("In main, task creation finished");
		try {
			cb.await();
		...
		System.out.println("Main await finshed.Now summing up the list");
		System.out.println("Size of List:" + list.size());
		int total=0;
		  for(int j=0;j<list.size();j++){
		  total+=list.get(j);
		  }
		  System.out.println("Total inside Main::" + total);
